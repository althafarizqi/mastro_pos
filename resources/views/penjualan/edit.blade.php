...
@section('content')
<form method="POST" action="/penjualan/{{$penjualanData->id}}">
    @csrf
    @method('PUT')
    <div class="row">
        ...
        <div class="col form-group">
            ...
            <input type="text" class="form-control form-control-sm" id="nota_penjualan" name="nota_penjualan"
                value="{{$penjualanData->nota_penjualan}}">
        </div>
        <div class="col form-group">
            <label for="exampleFormControlSelect2">Nama Pelanggan</label>
            <input type="hidden" value="{{$penjualanData->pelanggan->id}}" name="pelanggan_id">
            <input type="text" class="form-control form-control-sm" id="nota_penjualan"
                value="{{$penjualanData->pelanggan->nama}}">
        </div>
        <div class="col form-group">
            <label for="pembayaran">Jenis Pembayaran</label>
            <select class="form-control form-control-sm" id="pembayaran" name="pembayaran">
                <option value="{{$penjualanData->pembayaran}}">{{$penjualanData->pembayaran}}</option>
        ...
        <div class="col form-group">
            ...
            <input type="date" class="form-control form-control-sm" id="tanggal_japo" name="tanggal_japo"
                value="{{$penjualanData->tanggal_japo}}">
        </div>
    </div>
            ...
            <div class="table-responsive-sm">
                ...
                <table class="table table-sm table-borderless">
                    ...
                    <tbody id="data">
                        ...
                        <tr id="input-tr-{{ $loop->index }}">
                            <td>
                                <select data-penjualan-id="{{$penjualanData->id}}"
                                    class="form-control select2 form-control-sm" name="barang_id[]">
                                    <option value="">Pilih Barang</option>
                                    @foreach($detailPenjualanData as $barang)
                                    <option value="{{ $barang->barang->id }}" data-harga="{{ $barang->harga }}" {{
                                        $item->
                                        barang_id == $barang->barang->id ?
                                        'selected' : '' }}>{{ $barang->barang->nama }}</option>
                                    @endforeach
                                </select>
                            </td>
                            <td><input type="text" class="form-control form-control-sm" name="jumlah[]"
                                    value="{{ $item->jumlah }}"></td>
                            <td><input type="text" class="form-control form-control-sm" name="harga_satuan[]"
                                    value="{{ $item->harga_satuan }}">
                            </td>
                            <td><input type="text" class="form-control form-control-sm" name="subtotal_harga[]"
                                    value="{{ $item->subtotal_harga }}"></td>
                            <td>
                                <button class="btn btn-sm btn-danger delete-record float-right"
                                    data-id="{{ $loop->index }}">Hapus</button>
                            </td>
                        </tr>
                        @endforeach
                    <tfoot>
    ...
    <div class="row">
        <div class="col-4 input-group mb-3">
            ...
            <input type="number" class="form-control form-control-sm" value="{{$penjualanData->pajak}}">
        </div>
        <div class="col-4 input-group mb-3">
            <div class="input-group-prepend">
        ...
        <div class="col-4 input-group mb-3">
            ...
            <input type="text" id="total-harga-input-display" class="form-control form-control-sm"
                value="{{$penjualanData->total_harga}}">
        </div>
    </div>
    <button class="btn btn-sm btn-primary" type="submit">Simpan Perubahan</button>
    <input type="hidden" name="total_harga" id="total-harga-input" value="0" value="{{$penjualanData->total_harga}}">
    <input type="text" name="penjualanData_id" id="total-harga-input" value="{{$penjualanData->id}}">
